...
* **update:** hand Windows pip self-update off to a child process so `headroom update` can replace `headroom.exe` without tripping `WinError 32` mid-uninstall. Non-Windows and non-pip update paths stay synchronous, and the printed upgrade command remains the exact manual recovery command ([#1941](https://github.com/headroomlabs-ai/headroom/issues/1941)).
