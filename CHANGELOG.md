...
### Fixed
- **sdk/typescript:** carry hook-computed per-message compression biases through the TypeScript client and `/v1/compress` contract into the Python pipeline, with strict validation for indices and finite positive values instead of silently discarding caller policy.
