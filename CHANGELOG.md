...
### Fixed
- **proxy(rust)/drift-detector:** key the cache-bust drift detector on conversations instead of credentials, honor `x-headroom-session-id` (parity with the Python proxy's session identity), and canonicalize hashes before comparing (`cache_control` stripped outside opaque tool payloads, object keys sorted, per-message prefix-aware early-window comparison). Previously every concurrent conversation sharing one `Authorization`/`x-api-key` value alternated over a single LRU slot — each conversation switch logged a false `cache_drift_observed` — and a lone conversation's normal growth plus the client relocating its `cache_control` breakpoint fired a false `early_messages` drift on turn 2 of essentially every multi-turn session (verified against live Claude Code traffic). The conversation identity is `(model, first message)`, so per-model auxiliary calls reusing an opener stay separate. True positives are preserved: a settled early message changing or disappearing under a stable session key, and `system`/`tools` changes, still warn. Known trade-off: without the explicit header, clients that rewrite their first message (compaction, rolling-window history) re-key to a fresh session (`cache_drift_first_request`) rather than reporting drift against the old baseline.
