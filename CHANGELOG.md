...
* **proxy:** expose retry backoff timing through `--retry-base-delay-ms`, `--retry-max-delay-ms`, `HEADROOM_RETRY_BASE_DELAY_MS`, and `HEADROOM_RETRY_MAX_DELAY_MS`, threading the existing `ProxyConfig.retry_base_delay_ms` / `retry_max_delay_ms` knobs out to the CLI without changing their current defaults ([#2030](https://github.com/headroomlabs-ai/headroom/issues/2030)).
