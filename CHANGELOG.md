...
* **tokenizers:** count images nested inside `tool_result` content lists at their pixel-based cost (~1600 tokens) instead of json.dumps-ing the block and counting the base64 payload as text. Claude Code delivers screenshots exactly there (`tool_result` → `image` block), so a ~500KB screenshot counted as ~170K fake tokens — observed as ~12x request-level token inflation on screenshot-heavy sessions (local count 3.17M vs provider-reported ~260K), which zeroed displayed savings rates and falsely tripped size thresholds such as the `HEADROOM_BACKGROUND_COMPRESSION` cold-start gate. The `tool_result` branch now recurses into `_count_content_parts`, mirroring the Strands `toolResult` branch; non-list content keeps the serialized path.
