...
* **tokenizers:** bound `TiktokenCounter` token counting for oversized unhandled content parts. The fallback branch in `count_messages` now routes those parts through `BaseTokenizer._count_serialized`, which samples multi-megabyte blobs and serializes JSON wire form instead of feeding `count_text(str(part))` the whole Python repr synchronously. This keeps large tool-result payloads from stalling the event loop or tripping tiktoken regex backtracking while leaving normal-sized content exact.
