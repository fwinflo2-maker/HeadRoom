...
### Fixed
- **openclaw:** delegate persistent compaction to OpenClaw's built-in runtime instead of claiming compaction ownership and returning a successful no-op. Headroom continues to compress assembled model input, while `/compact` and overflow recovery now use OpenClaw's transcript-aware compaction path.
