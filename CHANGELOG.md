...
### Features
- **cli:** `headroom wrap droid` routes a Factory Droid session through the Headroom proxy on your Factory subscription, with no `customModels` edits and no API keys. Droid honours `FACTORY_API_BASE_URL`, so the wrapper points it at the local proxy: the Anthropic-shaped `/api/llm/a/v1/messages` inference route (used by every Droid model, including Droid Core) is compressed, while every other Factory REST path is forwarded verbatim to the real upstream with the session credential passed through untouched. The proxy gains `--factory-api-url` (env `FACTORY_TARGET_API_URL`) to select the upstream, defaulting to an existing `FACTORY_API_BASE_URL` or `https://api.factory.ai`.
