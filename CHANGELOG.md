...
* **packaging:** keep `headroom-ai[all]` installable on Intel macOS while preserving the same extra bundle. The Intel-macOS guard now skips only the unsupported torch-bearing requirements inside `ml`, `memory`, `evals`, and `voice`, while other platforms keep the current full dependency set ([#1931](https://github.com/headroomlabs-ai/headroom/issues/1931)).
