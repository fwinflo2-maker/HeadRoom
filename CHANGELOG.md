...
### Fixed
- **proxy:** quarantine request-path compression while a worker that exceeded its asyncio deadline is still running. Python cannot preempt an already-started executor thread, and the proxy previously only counted that timeout debt while continuing to admit more work; repeated slow requests could therefore occupy every compression worker and add the full timeout to each request. The first timeout follows the existing compression-failure policy, but subsequent compression attempts now bypass the executor immediately until all known post-timeout workers have exited, then recover automatically. `/health` and `/stats` expose the live quarantine/debt counters, and Prometheus exports activation/skip events ([#946](https://github.com/headroomlabs-ai/headroom/issues/946), [#810](https://github.com/headroomlabs-ai/headroom/issues/810)).
