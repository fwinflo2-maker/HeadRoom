...
### Fixed
- **memory/mcp:** publish the local memory backend only after embedder and vector-index warm-up completes. Concurrent tool calls now share and await one initialization task, while failed candidates are closed and cleared so later calls can retry instead of receiving a partially initialized backend.
