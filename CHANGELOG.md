...
### Features
- **proxy:** `POST /v1/compress` now accepts the full user-facing `CompressConfig` in the request `config` object — `compress_system_messages`, `min_tokens_to_compress`, and `frozen_message_count` join the previously supported `compress_user_messages`, `target_ratio`, `protect_recent`, and `protect_analysis_context`. Request-level keys override the server config / `HEADROOM_SAVINGS_PROFILE` posture only for the keys actually sent. Validation is strict per the no-silent-fallback constraint: unknown keys and mistyped or out-of-range values (e.g. `target_ratio` outside `(0, 1]`, negative counts, `"true"` instead of `true`) now return `400 invalid_request` with an actionable message instead of being silently dropped or half-coerced. `kompress_model` and `savings_profile` remain operator-level on purpose (arbitrary per-request model downloads / posture switching over HTTP).
