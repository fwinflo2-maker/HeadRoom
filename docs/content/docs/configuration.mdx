...
`--memory-cognee-data-root`, `--memory-cognee-search-type`, and
`--memory-cognee-auto-cognify/--no-memory-cognee-auto-cognify` (each also reads
the matching `HEADROOM_COGNEE_*` env var; the flag wins when both are set). See
