...
Install only what you need, or grab everything supported on your platform with `[all]`:
...
| `all` | Everything supported on your platform | `pip install "headroom-ai[all]"` |

On Intel macOS, `[all]` still includes the `ml`, `memory`, `evals`, and `voice` extras, but skips the unavailable torch-bearing requirements inside them.
