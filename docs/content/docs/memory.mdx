...
- cognee needs an LLM provider for the cognify graph-build step. Configure it through cognee's own settings (e.g. `LLM_API_KEY`; cognee reads its environment and `.env` file). If the key is missing or cognify fails, the save itself still succeeds -- the enrichment failure is logged, never raised. Headroom snapshots the process environment around cognee's import and restores any pre-existing variables its `.env` load overwrote, so a `.env` in the proxy's working directory cannot silently override live environment variables.
- cognee has **no per-item update or hard delete**. `update_memory` tombstones the old content and re-adds the new content as a fresh cognee data item; `delete_memory` tombstones. Tombstones are scoped per user and filtered out of this backend's search results by exact or substring match (best-effort: text cognee transformed during cognify may still surface), and the underlying data stays in cognee's stores until the dataset is pruned.
- Memory IDs are stable, content-derived UUIDs, so IDs returned by `memory_search` can be passed to `memory_update` / `memory_delete`. `get_memory`, `update_memory`, and `delete_memory` resolve memories saved or searched through the same backend instance (cognee has no fetch-by-id API for raw memories).
