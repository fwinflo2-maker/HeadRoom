...
datasource — it renders tokens saved, compression-ratio distribution, and latency
percentiles straight from the `headroom_*` metrics above.

It is built entirely on the documented metric names, so the same queries below drive it:
