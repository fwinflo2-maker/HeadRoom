...
| `--code-graph` | `false` | Enable code-graph indexing for the current project. |
| `--code-graph-backend <backend>` | `codebase-memory-mcp` | Select `codebase-memory-mcp` or `codegraph`. |

#### Code-graph backend selection

`--code-graph` enables project indexing. The backend can be selected with
`--code-graph-backend`, `HEADROOM_CODE_GRAPH_BACKEND`, or a `code_graph_backend`
key in `headroom.toml` or `headroom.yaml`. The precedence is CLI, environment,
config file, then the current default (`codebase-memory-mcp`).

CodeGraph is installed through npm and initialized in the current project:
...
Headroom runs `codegraph install` to register the detected agent and
`codegraph init` to create the project's `.codegraph/` index. CodeGraph keeps
its own file watcher active; Headroom's watchdog watcher remains for the
`codebase-memory-mcp` backend. `headroom unwrap <agent>` and
`headroom mcp uninstall` remove CodeGraph's agent configuration while keeping
the CLI installed.
