...
`token_budget` (optional) overrides the model's context limit as the
compression target. `config` (optional) exposes the user-facing
`CompressConfig` knobs per request; any key left unset falls back to the
server-level configuration (including `HEADROOM_SAVINGS_PROFILE`), and
request-level keys win over it. Validation is strict: unknown keys or
mistyped values return `400 invalid_request` instead of being silently
ignored. `kompress_model` and `savings_profile` are deliberately not
accepted over HTTP — model selection and the savings posture stay
operator-level settings.

| `config` key | Type | Constraint |
|---|---|---|
| `compress_user_messages` | boolean | — |
| `compress_system_messages` | boolean | — |
| `protect_analysis_context` | boolean | — |
| `protect_recent` | integer | ≥ 0 |
| `frozen_message_count` | integer | ≥ 0 |
| `min_tokens_to_compress` | integer | ≥ 0 |
| `target_ratio` | number | in `(0, 1]` |
