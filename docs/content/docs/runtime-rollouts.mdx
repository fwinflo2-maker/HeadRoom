...
An unknown channel logs a warning and falls back to `stable`. Unknown feature
names from the environment log a warning and are ignored (fail-closed). Explicit
CLI diagnostics reject unknown channels/features and list the valid features.
...
to their own runtimes, but share channel ordering, precedence, reasons, invalid
input handling, and deterministic identity semantics.
