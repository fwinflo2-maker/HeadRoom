# Gemini 3.7 Flash on Google Cloud Vertex AI + Headroom Benchmark

A reproducible, real-world benchmark evaluating **Headroom context compression** with **Gemini 3.7 Flash** on **Google Cloud Vertex AI (Gemini Enterprise Agent Platform)**.
...
Even with Gemini 3.7 Flash's massive context window and fast inference, bloated tool returns:
...
Tested live on **Google Cloud Vertex AI** (`global` endpoint) with **`gemini-3.7-flash`**:
...
| **BigQuery Table Analytics** | 500 Tabular Transaction Rows | 49,020 | 38,504 | **-21.5%** | 100% (✓ PASS) |
| **Multi-Turn RAG Synthesis** | 25 Dense Specification Chunks | 4,071 | 1,521 | **-62.6%** | 100% (✓ PASS) |
| **TOTAL / AGGREGATE** | **Real-World Agent Trajectory** | **111,679** | **58,576** | **-47.5%** | **100% Preserved** |

### Key Metrics Summary

* **Prompt Tokens Saved**: **53,103 tokens** (**47.5% net reduction**)
* **Inference Cost Reduction**: **43.6% savings** on Vertex AI standard tier
...
```
                                          ...
                                          │                                       │
┌───────────────────────┐                 │  ┌─────────────────────────────────┐  │
│  google-genai Python  │                 │  │        gemini-3.7-flash         │  │
│  SDK Agent / Script   │                 │  └─────────────────────────────────┘  │
└───────────┬───────────┘                 └───────────────────▲───────────────────┘
            │                                                 │
...
--model           Vertex model ID (default: gemini-3.7-flash)
...
📉 Results:
• Prompt Token Reduction: 47.5% (111,679 ➔ 58,576 tokens)
• SRE Log Scenario Reduction: 73.1% (51.7k ➔ 13.9k tokens)
• Total Cost Savings: 43.6%
