...
class ImageCompressor:
    ...
    def _get_onnx_router(self) -> Any:
        """Return the process-wide shared ONNX technique router.

        Building an ``OnnxTechniqueRouter`` loads native ``ort.InferenceSession``
        models. The registry instance is also preloaded during proxy startup, so
        requests reuse the warm sessions instead of maintaining a second cache.
        """
        from headroom.models.ml_models import MLModelRegistry

        return MLModelRegistry.get_onnx_technique_router(use_siglip=self.use_siglip)
