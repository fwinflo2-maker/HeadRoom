...
def conversation_key_from_responses_body(body: dict[str, Any]) -> str:
    ...
    The Responses counterpart of :func:`conversation_key_from_body`: hashes
    the model plus the first user message's text from ``input``. That first
    turn is immutable for the conversation's lifetime, so the whole
    conversation lands in one holdout arm.
    """
    model = str(body.get("model", ""))
    seed = model
