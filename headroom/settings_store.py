...
SETTINGS: tuple[SettingField, ...] = (
    # --- Compression ---
    SettingField(
        "HEADROOM_SAVINGS_PROFILE", "savings_profile", "Savings profile",
        "Compression", "enum", default="coding",
        choices=("agent-90", "balanced", "coding", "general"),
        help="Named compression posture applied at startup.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_TARGET_RATIO", "target_ratio", "Target keep-ratio",
        "Compression", "float", default=None, minimum=0.0, maximum=1.0,
        help="Kompress keep-ratio 0-1 (lower = more aggressive). Unset = adaptive.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_DISABLE_KOMPRESS", "disable_kompress", "Disable Kompress",
        "Compression", "bool", default=False,
        help="Disable Kompress ML compression (structural compression stays on).",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_LOSSLESS", "lossless", "Lossless mode",
        "Compression", "bool", default=False,
        help="No-CCR lossless compaction; no retrieval marker emitted.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_CODE_AWARE_ENABLED", "code_aware_enabled", "Code-aware compression",
        "Compression", "bool", default=True,
        help="AST-based code compression (requires the [code] extra).",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_PROTECT_TOOL_RESULTS", "protect_tool_results", "Protect tool results",
        "Compression", "str", default=None,
        help="Comma-separated tool names whose results are never lossy-compressed.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_NO_CCR", "no_ccr", "Disable CCR",
        "Compression", "bool", default=False,
    ...
    # --- Limits ---
    SettingField(
        "HEADROOM_RPM", "rpm", "Requests / min",
        "Limits", "int", default=None, minimum=1,
        help="Max requests per minute.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_TPM", "tpm", "Tokens / min",
        "Limits", "int", default=None, minimum=1,
        help="Max tokens per minute.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_LIMIT_CONCURRENCY", "limit_concurrency", "Concurrency limit",
        "Limits", "int", default=1000, minimum=1,
        help="Max concurrent connections before Uvicorn returns 503.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_WORKERS", "workers", "Worker processes",
        "Limits", "int", default=1, minimum=1,
        help="Uvicorn worker processes.",
        tier="advanced",
    ),
    # --- Budget ---
    SettingField(
        "HEADROOM_BUDGET", "budget", "Budget (USD)",
        "Budget", "float", default=None, minimum=0.0,
        help="Budget limit per period; requests are rejected with 429 once reached.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_BUDGET_PERIOD", "budget_period", "Budget period",
        "Budget", "enum", default="daily",
    ...
    SettingField(
        "HEADROOM_HOST", "host", "Host",
        "Networking", "str", default="127.0.0.1", manifest_managed=True,
        help="Bind host. Managed by the install manifest on docker/service installs.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_PORT", "port", "Port",
        "Networking", "int", default=8787, minimum=1, maximum=65535,
    ...
    # --- Logging ---
    SettingField(
        "HEADROOM_LOG_MESSAGES", "log_messages", "Log message content",
        "Logging", "bool", default=False,
        help="Log full request/response content. WARNING: may log sensitive data.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_LOG_FILE", "log_file", "Log file path",
        "Logging", "str", default=None,
        help="Path for the message log file.",
        tier="basic",
    ),
    # --- Networking (upstream connection pool tuning) ---
    SettingField(
        "HEADROOM_MAX_CONNECTIONS", "max_connections", "Max upstream connections",
        "Networking", "int", default=500, minimum=1,
        help="Maximum upstream HTTP connections in the shared httpx pool.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_MAX_KEEPALIVE", "max_keepalive_connections", "Max keep-alive connections",
        "Networking", "int", default=100, minimum=0,
        help="Maximum upstream keep-alive connections in the shared httpx pool.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_HTTP2", "http2", "HTTP/2 upstream",
        "Networking", "bool", default=True,
        help="Use HTTP/2 for upstream provider connections.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_HTTP_PROXY", "http_proxy", "Outbound HTTP proxy",
        "Networking", "str", default=None,
        help="HTTP proxy URL for upstream provider requests only (HTTPS uses CONNECT).",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_KEEPALIVE_EXPIRY", "keepalive_expiry", "Keep-alive expiry (s)",
        "Networking", "float", default=90.0, minimum=0.0,
        help="Seconds an idle upstream keep-alive connection is kept open.",
        tier="advanced",
    ),
    # --- Compression (additional internals) ---
    SettingField(
        "HEADROOM_NO_CCR_PROACTIVE_EXPANSION", "no_ccr_proactive_expansion", "Disable CCR proactive expansion",
        "Compression", "bool", default=False,
        help="Disable proactive expansion of previously compressed content.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_COMPRESSION_MAX_WORKERS", "compression_max_workers", "Compression worker pool size",
        "Compression", "int", default=None,
        help="Bound the dedicated compression threadpool (CPU-bound Kompress work). Unset = cpu_count.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_DISABLE_KOMPRESS_FALLBACK", "disable_kompress_fallback", "Disable Kompress fallback",
        "Compression", "bool", default=False,
        help="With disable-kompress, route fall-through content to passthrough instead of the Kompress fallback.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_DISABLE_KOMPRESS_ANTHROPIC", "disable_kompress_anthropic", "Disable Kompress (Anthropic)",
        "Compression", "optional-bool", default=None,
        help="Disable (false) or force-enable (true) Kompress for the Anthropic pipeline only. Unset = inherit.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_DISABLE_KOMPRESS_OPENAI", "disable_kompress_openai", "Disable Kompress (OpenAI)",
        "Compression", "optional-bool", default=None,
    ...
    SettingField(
        "HEADROOM_READ_MATURATION", "read_maturation", "Read maturation",
        "CCR", "bool", default=False,
        help="EXPERIMENTAL: hold fresh Reads out of compression until the file quiesces.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_READ_MATURATION_QUIESCE_TURNS", "read_maturation_quiesce_turns", "Maturation quiesce turns",
        "CCR", "int", default=5, minimum=1,
        help="Turns a file must stay quiet before a held Read is matured.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_READ_MATURATION_MAX_HOLD_TURNS", "read_maturation_max_hold_turns", "Maturation max hold turns",
        "CCR", "int", default=25, minimum=1,
        help="Force-mature a held Read after this many turns even if the file stays active.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_READ_MATURATION_MIN_SIZE_BYTES", "read_maturation_min_size_bytes", "Maturation min size (bytes)",
        "CCR", "int", default=2048, minimum=0,
        help="Only hold/mature Read outputs at least this many bytes.",
        tier="advanced",
    ),
    # --- Extensions ---
    SettingField(
        "HEADROOM_PROXY_EXTENSIONS", "proxy_extensions", "Enabled proxy extensions",
        "Extensions", "csv-list", default=None,
        help="Comma-separated opt-in proxy extension entry-point names ('*' enables all discovered).",
        tier="advanced",
    ),
    # --- Backend ---
    SettingField(
        "HEADROOM_NO_SUBSCRIPTION_TRACKING", "no_subscription_tracking", "Disable subscription tracking",
        "Backend", "bool", default=False,
        help="Disable the Anthropic Claude subscription usage poller (GET /api/oauth/usage).",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_SUBSCRIPTION_POLL_INTERVAL", "subscription_poll_interval", "Subscription poll interval (s)",
        "Backend", "int", default=None, minimum=1, maximum=3600,
        help="Seconds between Anthropic subscription usage polls. Default: 300.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_BACKEND", "backend", "Upstream backend",
        "Backend", "str", default="anthropic",
        help="API backend: anthropic, bedrock, openrouter, anyllm, or litellm-<provider>.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_ANYLLM_PROVIDER", "anyllm_provider", "any-llm provider",
        "Backend", "str", default="openai",
        help="Provider for the any-llm backend: openai, mistral, groq, ollama, etc.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_REGION", "region", "Cloud region",
        "Backend", "str", default="us-west-2",
        help="Cloud region for Bedrock/Vertex/etc backends.",
        tier="advanced",
    ),
    # --- Timeouts ---
    SettingField(
        "HEADROOM_RETRY_MAX_ATTEMPTS", "retry_max_attempts", "Upstream retry attempts",
        "Timeouts", "int", default=None, minimum=1, maximum=10,
    ...
    SettingField(
        "HEADROOM_CONNECT_TIMEOUT_SECONDS", "connect_timeout_seconds", "Connect timeout (s)",
        "Timeouts", "int", default=None, minimum=1, maximum=300,
        help="Upstream connection timeout in seconds. Default: 10.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS", "anthropic_buffered_request_timeout_seconds",
        "Anthropic buffered timeout (s)",
        "Timeouts", "int", default=None, minimum=1,
        help="Buffered Anthropic read timeout for non-streaming message batch paths. Default: 600.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY", "anthropic_pre_upstream_concurrency",
    ...
    SettingField(
        "HEADROOM_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS", "anthropic_pre_upstream_acquire_timeout_seconds",
        "Pre-upstream acquire timeout (s)",
        "Timeouts", "float", default=None,
    ...
    # --- Memory ---
    SettingField(
        "HEADROOM_MEMORY_DB_PATH", "memory_db_path", "Memory DB path",
        "Memory", "str", default="",
        help="Path to the legacy single-file memory SQLite DB. Default: {cwd}/.headroom/memory.db.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_MEMORY_PROJECT_ROOT", "memory_project_root", "Memory project root",
        "Memory", "str", default="",
        help="Override the project root used for --memory-storage=project.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_NO_MEMORY_TOOLS", "no_memory_tools", "Disable memory tools",
        "Memory", "bool", default=False,
        help="Disable automatic injection of memory_save/memory_search tools.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_NO_MEMORY_CONTEXT", "no_memory_context", "Disable memory context injection",
        "Memory", "bool", default=False,
        help="Disable automatic injection of relevant memories into the system prompt.",
        tier="basic",
    ),
    SettingField(
        "HEADROOM_MEMORY_TOP_K", "memory_top_k", "Memory retrieval top-K",
        "Memory", "int", default=10, minimum=1, maximum=100,
        help="Number of semantically-relevant memories to retrieve.",
        tier="advanced",
    ),
    SettingField(
        "HEADROOM_MIN_EVIDENCE", "min_evidence", "Minimum evidence count",
        "Memory", "int", default=None, minimum=1,
    ...
    SettingField(
        "ANTHROPIC_TARGET_API_URL", "anthropic_base_url", "Anthropic base URL",
        "Endpoints", "str", default=None,
        help="Custom Anthropic API base URL (e.g. Azure Foundry, corporate gateway). Overrides https://api.anthropic.com.",
        tier="advanced",
    ),
    SettingField(
        "OPENAI_TARGET_API_URL", "openai_base_url", "OpenAI base URL",
        "Endpoints", "str", default=None,
        help="Custom OpenAI API base URL (e.g. corporate gateway). Overrides https://api.openai.com.",
        tier="advanced",
    ),
    SettingField(
        "ANTHROPIC_TARGET_API_HEADERS", "anthropic_extra_headers", "Anthropic extra headers",
        "Endpoints", "header-map", default=None, secret=True,
        help='JSON object of extra headers merged into (and overriding) forwarded Anthropic requests, e.g. {"Api-Key": "..."}.',
        tier="advanced",
    ),
    SettingField(
        "OPENAI_TARGET_API_HEADERS", "openai_extra_headers", "OpenAI extra headers",
        "Endpoints", "header-map", default=None, secret=True,
