...
const CCR_MARKER_RE =
  /^.*(?:Retrieve more: hash=|Retrieve original: hash=|<<ccr:[^>]+>>|<headroom:[^>]+>).*$/m;
...
function hasPruningMetadata(details: unknown): boolean {
  ...
  if ("prunedAt" in details || ("pruned" in details && details.pruned === true)) {
    return true;
  }
...
export function transformContext<TMessage extends ContextMessage>(
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    if (!message || !isToolResultMessage(message) || protectedIndexes.has(index)) {
      continue;
    }
