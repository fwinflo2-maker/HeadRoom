...
import { buildCompressPayload, validateCompression } from "../src/bridge.js";
...
describe("validateCompression", () => {
  ...
  it("rejects a response for a different synthetic tool", async () => {
    const raw = response();
    const assistant = (raw.messages as Array<{
  ...
  it.each([
    { label: "missing messages", mutate: (raw: Record<string, unknown>) => delete raw.messages },
    { label: "wrong message count", mutate: (raw: Record<string, unknown>) => (raw.messages = []) },
    {
    ...
    },
    {
      label: "wrong tool id",
      mutate: (raw: Record<string, unknown>) =>
        (((raw.messages as Record<string, unknown>[])[1] ?? {}).tool_call_id = "wrong"),
    },
    {
      label: "empty output",
      mutate: (raw: Record<string, unknown>) =>
    ...
    },
    {
      ...
      mutate: (raw: Record<string, unknown>) =>
        (((raw.messages as Record<string, unknown>[])[1] ?? {}).content = "x".repeat(candidate.originalText.length)),
    },
    {
      label: "insufficient savings",
      mutate: (raw: Record<string, unknown>) => (raw.tokens_saved = 255),
    },
    {
  ...
  it("rejects a nonempty CCR retrieval outside the source result", async () => {
    await expect(
      validateCompression(candidate, response(), async () => "unrelated payload"),
    ).resolves.toBeUndefined();
  });
});
