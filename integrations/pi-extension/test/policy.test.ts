...
describe("candidateFromToolResult", () => {
  it("protects configured tools and error results", () => {
    expect(candidateFromToolResult(toolResult("1", "READ", "x".repeat(20)), config())).toBeUndefined();
    expect(
      candidateFromToolResult(
  ...
  it("rejects short and already-compressed output", () => {
    expect(candidateFromToolResult(toolResult("1", "bash", "short"), config())).toBeUndefined();
    expect(
      candidateFromToolResult(
        toolResult(
          "2",
...
describe("protectedToolResultIndexes", () => {
  it("protects the newest results by message position", () => {
    const messages = [
      ...
      { role: "custom", customType: "gap", content: "gap", display: false, timestamp: 2 },
...
describe("transformContext", () => {
  ...
  it("copies only the substituted message and text block", () => {
    ...
    expect((result?.[0] as ToolResultMessage).content[0]).not.toBe(old.content[0]);
