...
| latest Phase 9-B provider commit | 5667be8 Document Codex custom provider validation |
| latest source Markdown refresh commit | 1e9da9f4 Document source Markdown refresh rules |
| latest local commit確認 | git log -1 --oneline で確認する |
| push | 未実行 |
...
`03_ROADMAP_PROGRESS.md` の現在状態テーブルでは、曖昧な `latest commit` 表記を避ける。

記録する場合は、次のように意味を分ける。

- `latest Phase work commit`
- `latest docs commit`
- `latest source Markdown refresh commit`

注意:

- docs commit自身のhashを、そのcommit内のMarkdownへ完全に記録することはできない。
- 最新HEADは `git log -1 --oneline` を正とする。
- Phase完了時は、現在状態テーブルに古いcommit情報が残っていないか確認する。
