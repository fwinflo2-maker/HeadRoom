...
def build_dashboard(entries: list[dict]) -> dict[str, Any]:
    ...
    by_provider: dict[str, dict] = defaultdict(
        lambda: {"tokens_saved": 0, "tokens_original": 0, "count": 0, "savings_pct": 0.0}
    )
    by_type: dict[str, dict] = defaultdict(
        lambda: {"tokens_saved": 0, "tokens_original": 0, "count": 0}
    )
...
def main():
    ...
    if not log_files:
        ...
        print(
            "Example: python3 bench/savings_dashboard.py --log-file /tmp/headroom-c3/requests.jsonl",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if args.json:
        ...
        print(
            f"Impact: {dashboard['environment']['co2_kg_saved']:.4f} kg CO2 saved, "
        ...
        for ctype, data in dashboard["by_content_type"].items():
            print(
                f"{ctype:<20} {data['savings_pct']:>7.1f}% {data['tokens_saved']:>10,} {data['count']:>8}"
            )
