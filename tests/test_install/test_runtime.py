...
def test_agent_pid_round_trip(monkeypatch, tmp_path: Path) -> None:
    import headroom.install.runtime as runtime

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    runtime._write_agent_pid("default", 654)
    assert runtime._read_agent_pid("default") == 654
    runtime._clear_agent_pid("default")
    assert runtime._read_agent_pid("default") is None


def test_stop_runtime_terminates_agent_process_group(monkeypatch, tmp_path: Path) -> None:
    import headroom.install.runtime as runtime

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr(runtime, "_is_windows", lambda: False)
    manifest = _python_service_manifest()
    runtime._write_agent_pid("default", 122)
    runtime._write_pid("default", 123)
    groups: list[tuple[int, int]] = []
    pids: list[tuple[int, int]] = []
    monkeypatch.setattr(runtime.os, "killpg", lambda pid, sig: groups.append((pid, sig)))
    monkeypatch.setattr(runtime.os, "kill", lambda pid, sig: pids.append((pid, sig)))

    runtime.stop_runtime(manifest)

    assert groups == [(122, signal.SIGTERM)]
    assert pids == [(123, signal.SIGTERM)]
    assert runtime._read_agent_pid("default") is None
    assert runtime._read_pid("default") is None
