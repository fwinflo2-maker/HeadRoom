...
def _claude_code_tools(*, with_tool_search: bool = False) -> list[dict]:
    """Claude Code's surface: PascalCase built-ins + MCP tools.

    ``with_tool_search`` adds the client's own ``ToolSearch``, which signals that
    the client defers client-side and turns injection into a no-op — so the tests
    that exercise deferral itself leave it out.
    """
    names = ["Bash", "Read", "Write", "Edit", "Glob", "Grep", "Task", "WebFetch"]
    if with_tool_search:
        names.append("ToolSearch")
    names += [f"mcp__srv__t{i}" for i in range(12)]
...
def test_core_tools_match_case_insensitively() -> None:
    ...
    # name the same tools Read/Edit/Bash. Without a case-insensitive match, routine
    # edit/read/run loops each pay a search round-trip — the exact thing
    # _TOOL_SEARCH_CORE_TOOLS exists to avoid.
    out = inject_tool_search_deferral(_claude_code_tools())
    by_name = {t.get("name"): t for t in out if "name" in t}
    for name in ("Bash", "Read", "Write", "Edit", "Glob", "Grep", "Task", "WebFetch"):
...
def test_noop_when_client_defers_client_side_via_toolsearch() -> None:
    ...
    tools = _claude_code_tools(with_tool_search=True)
    assert inject_tool_search_deferral(tools) is tools
...
def test_client_tool_search_tool_is_never_deferred() -> None:
    ...
    # tool that can load them, so they become permanently unreachable. It is both
    # a core (never-deferred) name and a client-side-deferral signal; assert the
    # tool survives however the guards happen to fire.
    out = inject_tool_search_deferral(_claude_code_tools(with_tool_search=True))
    by_name = {t.get("name"): t for t in out if "name" in t}
